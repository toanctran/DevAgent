...
from devagent.chat_env import ChatEnv, ChatEnvConfig
from devagent.statistics import get_info
from devagent.utils import log_and_print_online, now
...
class ChatChain:
    ...
    def __init__(self,
        ...
        # import all used phases in PhaseConfig.json from devagent.phase
    ...
    def execute_step(self, phase_item: dict):
        ...
        if phase_type == "SimplePhase":
            ...
            if phase in self.phases:
                ...
                raise RuntimeError(f"Phase '{phase}' is not yet implemented in devagent.phase")
        # For ComposedPhase, we create instance here then conduct the "ComposedPhase.execute" method
            ...
            if not compose_phase_class:
                raise RuntimeError(f"Phase '{phase}' is not yet implemented in devagent.compose_phase")
    ...
    def pre_processing(self):
        ...
        preprocess_msg += "**TT DevAgent Starts** ({})\n\n".format(self.start_time)
    ...
    def post_processing(self):
        ...
        dt1 = datetime1.strftime("%y%b%d, %H:%M%S")
        dt2 = datetime2.strftime("%y%b%d, %H:%M%S")
        post_info += "Software Info: {}".format(
            get_info(self.chat_env.env_dict['directory'], self.log_filepath) + "\n\n🕑**duration**={:.2f}s\n\n".format(
                duration))

        post_info += "TT DevAgent Starts ({})".format(dt1) + "\n\n"
        post_info += "TT DevAgent Ends ({})".format(dt2) + "\n\n"
